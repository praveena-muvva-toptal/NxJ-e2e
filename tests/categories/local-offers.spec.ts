...
describe('Verifying Local Offers Category', () => {

  const testData = require('../../test-data/categories.testdata.json');
  const CategoryData = new Category(testData.US.localOffers);

  beforeEach(async() => {
     await CategoriesPage.navigateTo();
  });

  it('TC #1: Should navigate to Local Offers category page', async () => {
    await CategoriesPage.navigateTo();
