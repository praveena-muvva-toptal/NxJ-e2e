...
describe('Verifying movie tickets page Navigation', () => {

  const testData = require('../../test-data/categories.testdata.json');
  const CategoryData = new Category(testData.US.movieTickets);

  beforeEach(async() => {
     await CategoriesPage.navigateTo();
  });

  it('TC #1: Should navigate to movie tickets category page', async () => {
    await CategoriesPage.navigateTo();
